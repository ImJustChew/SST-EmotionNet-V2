...
from tensorflow.python.keras.layers.pooling import (AveragePooling3D, GlobalAveragePooling3D,
                                  MaxPooling3D)
...
def __conv_block(ip, nb_filter, bottleneck=False, dropout_rate=None, weight_decay=1e-4):
    ...
    x = tf.keras.layers.BatchNormalization(axis=concat_axis, epsilon=1.1e-5)(ip)
    x = Activation('relu')(x)
...
def __create_dense_net(nb_classes, img_input, include_top, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1,
    ...
    if subsample_initial_block:
        x = tf.compat.v1.keras.layers.BatchNormalization(axis=concat_axis, epsilon=1.1e-5)(x)
        x = Activation('relu')(x)
...
def Attention_block(input_tensor, spatial_attention=True, temporal_attention=True):
    ...
    x = tf.keras.layers.Reshape([K.int_shape(input_tensor)[1], K.int_shape(
    ...
    if spatial_attention:
        ...
        spatial = tf.keras.layers.Flatten()(spatial)
        spatial = Dense(nbSpatial)(spatial)
        spatial = Activation('sigmoid')(spatial) # should be softmax, specified in the paper
        spatial = tf.keras.layers.Reshape(
            [K.int_shape(input_tensor)[1], K.int_shape(input_tensor)[2], 1, 1])(spatial)

        tem = tf.keras.layers.multiply([input_tensor, spatial])

    if temporal_attention:
        temporal = AveragePooling3D(pool_size=[K.int_shape(input_tensor)[
                                    1], K.int_shape(input_tensor)[2], 1])(x)
        temporal = tf.keras.layers.Flatten()(temporal)
        temporal = Dense(nbTemporal)(temporal)
        temporal = Activation('sigmoid')(temporal) # should be softmax, specified in the paper
        temporal = tf.keras.layers.Reshape(
            [1, 1, K.int_shape(input_tensor)[-2], 1])(temporal)

        tem = tf.keras.layers.multiply([temporal, tem])

    return tem


def sst_emotionnet(input_width, specInput_length, temInput_length, depth_spec, depth_tem, gr_spec, gr_tem, nb_dense_block, attention=True, spatial_attention=True, temporal_attention=True, nb_class=3, dense_dropout=0):
    '''
    ...
    x_s = __create_dense_net(img_input=specInput, depth=depth_spec, nb_dense_block=nb_dense_block,
                             growth_rate=gr_spec, nb_classes=nb_class, reduction=0.5, dropout_rate=dense_dropout, bottleneck=True, include_top=False, attention=attention, spatial_attention=spatial_attention, temporal_attention=temporal_attention)
    ...
    x_t = __create_dense_net(img_input=temInput, depth=depth_tem, nb_dense_block=nb_dense_block,
                             growth_rate=gr_tem, nb_classes=nb_class, bottleneck=True, dropout_rate=dense_dropout, include_top=False, subsample_initial_block=True, attention=attention)

    y = tf.keras.layers.concatenate([x_s, x_t], axis=-1)
    y = tf.keras.layers.Dense(50)(y)
    y = tf.keras.layers.Dropout(0.5)(y)
    if nb_class == 2:
        y = tf.keras.layers.Dense(nb_class, activation='sigmoid')(y)
    else:
        y = tf.keras.layers.Dense(nb_class, activation='softmax')(y)
