...
def run():
    ...
    train_specInput = np.zeros((0, input_width, input_width, specInput_length, 1))
    train_tempInput = np.zeros((0, input_width, input_width, temInput_length, 1))
    train_label = np.zeros((0))

    test_specInput = np.zeros((0, input_width, input_width, specInput_length, 1))
    test_tempInput = np.zeros((0, input_width, input_width, temInput_length, 1))
    ...
    model = sst_model.sst_emotionnet(input_width=input_width, specInput_length=specInput_length, temInput_length=temInput_length,
                                    depth_spec=depth_spec, depth_tem=depth_tem, gr_spec=gr_spec, gr_tem=gr_tem, nb_dense_block=nb_dense_block, nb_class=nb_class, dense_dropout=0.1)
    ...
    save_model = ModelCheckpoint(
        filepath=os.path.join(model_save_path, f'model.h5'),
    ...
    # Result Processing
    with open(os.path.join(result_path, f'model.txt'), "w") as f:
